...
class Temporal2D(TemporalYDep):
    ...
    def cross_correlate_pulse_front(self):
        ...
        self.pulse_front_cross_correlation=dict(t=t,fit_weight=fit_weight,linear_fit=linear_fit,linear_fit_coeffs=linear_fit_coeffs)
        
class PolarSpectral(Result):
    axis_attrs=Result.axis_attrs+('omega_0_ind',)
    array_attrs=Result.array_attrs+('If','phif','Ef','Et','omega_centroid','omega_hm0','omega_hm1','fwhm','phif_taylor')
...
class PolarSpectralYDep(PolarSpectral,YDependent):
    ...
    def oversample_y(self,factor):
        ...
        Ifo=abs(Efo)**2
        phifo=np.angle(Efo)
        phifo_orig=interp1d(self.yp.squeeze(),self.phif,copy=False,axis=AXIS_Y,fill_value='extrapolate')(ypo.squeeze())
        phifo+=2*math.pi*((phifo_orig-phifo)/(2*math.pi)).round()
